...
var createPresets = function createPresets() {
    return isTest ? [require.resolve('babel-preset-flow'), ['env', {
        targets: {
            browsers: ['last 2 versions', 'ie >= 9', '> 2%', 'ff >= 44', 'ios 8'],
            node: 4
        },
        loose: true
    }]] : [require.resolve('babel-preset-flow'), ['env', {
        modules: false,
        targets: {
            browsers: ['last 2 versions', 'ie >= 9', '> 2%', 'ff >= 44', 'ios 8'],
            node: 4
...
var config = {
    ...
    env: {
        server: {
            plugins: [require.resolve('babel-plugin-transform-es2015-modules-commonjs'), [require.resolve('babel-plugin-css-modules-transform'), {
